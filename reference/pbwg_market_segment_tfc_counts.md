...
Counts Network Manager area flights per day and market segment,
filtering the flights fact to operational statuses (`TE`, `TA`, `AA`)
and the requested date window. Segments are resolved through
`SWH_FCT.DIM_FLIGHT_TYPE_RULE`, and missing day/segment combinations are
backfilled with zeros to give a complete calendar grid.
